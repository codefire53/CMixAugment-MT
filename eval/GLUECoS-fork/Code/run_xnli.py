...
class GLUECoSNLIProcessor(processors['xnli']):
    def get_train_examples(self, data_dir, filename):
        ...
        lines = self._read_tsv(os.path.join(data_dir, f"XNLI-MT-1.0/multinli/{filename}"))
        examples = []
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        acc_scores = accuracy_score(out_label_ids, preds)

        logger.info("***** Eval results {} *****".format(prefix))
        logger.info(f"Accuracy: {acc_scores}")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        examples = (
            processor.get_test_examples(args.data_dir) if evaluate else processor.get_train_examples(args.data_dir, args.train_filename)
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Filename of the training data. Should be in .tsv file format"
    )
    parser.add_argument(
        "--model_type",
        default=None,
